...
point =
    succeed DataPoint
        ...
        |. (getChompedString <| chompWhile (\c -> not (Char.isAlpha c)))
        |= (getChompedString <| chompWhile Char.isAlpha)
        |. end


policy1 : Result error DataPoint -> Bool
policy1 res_entry =
    case res_entry of
        Ok entry ->
            String.filter (\n -> String.fromChar n == entry.char) entry.password |> String.length |> (\c -> c >= entry.min && c <= entry.max)

        _ ->
            False


policy2 : Result error DataPoint -> Bool
policy2 res_entry =
    case res_entry of
        Ok entry ->
            entry.char
                == String.slice (entry.min - 1) (entry.min + 0) entry.password
                && not (entry.char == String.slice (entry.max - 1) (entry.max + 0) entry.password)
                || not
                    (entry.char
                        == String.slice (entry.min - 1) (entry.min + 0) entry.password
                    )
                && entry.char
                == String.slice (entry.max - 1) (entry.max + 0) entry.password

        _ ->
            False


answer21 : List String -> Int
answer21 l =
    l |> List.map (\n -> Parser.run point n) |> List.filter (\n -> policy1 n) |> List.length


answer22 : List String -> Int
answer22 l =
    l |> List.map (\n -> Parser.run point n) |> List.filter (\n -> policy2 n) |> List.length
